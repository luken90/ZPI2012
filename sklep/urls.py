...
urlpatterns = patterns('',
    ...
    url(r'^produkty/rosnace/$', 'django.views.generic.list_detail.object_list', {'queryset': Towary.objects.filter(ilosc_w_sklepie__gt=0).select_related('kategorie').order_by('cena_sklepowa'), 'paginate_by': 8}, "sklep_produkty_rosnace"),
    url(r'^produkty/malejace/$', 'django.views.generic.list_detail.object_list', {'queryset': Towary.objects.filter(ilosc_w_sklepie__gt=0).select_related('kategorie').order_by('-cena_sklepowa'), 'paginate_by': 8}, "sklep_produkty_malejace"),
    ...
    url(r'^produkty/(\d+)/rosnace/$', 'sklep.views.produkty_z_kategoriirosnace', name="sklep_kategoria_rosnace"),
    url(r'^produkty/(\d+)/malejace/$', 'sklep.views.produkty_z_kategoriimalejace', name="sklep_kategoria_malejace"),
